...
simulate_continuous_outcome <- function(nsubj = 400,
  ...
  dat[, tt := t0 + 12]
  arms <- length(means)
  out <- vapply(seq_len(arms), function(j) {
    rnorm(nsubj, means[j], sigma)
  }, FUN.VALUE = matrix(0, nsubj, 1))
...
#' @export
simulate_trial_with_control <- function(n_seq = c(100, 200, 300, 400),
...
#' @return A list (or data.table) giving the trial results
#' @export
simulate_trial_with_control2 <- function(n_seq = c(100, 200, 300, 400),
                                         dat = simulate_continuous_outcome(nsubj = max(n_seq)),
